...
theorem mul_length_le_sum_of_min?_eq_some_int {xs : Array Int} (h : xs.min? = some x) :
    x * xs.size ≤ xs.sum := by
  rcases xs with ⟨l⟩
  simpa [List.min?_toArray, List.sum_toArray] using List.mul_length_le_sum_of_min?_eq_some_int h
...
theorem le_sum_div_length_of_min?_eq_some_int {xs : Array Int} (h : xs.min? = some x) :
    x ≤ xs.sum / xs.size := by
  rcases xs with ⟨l⟩
  simpa [List.min?_toArray, List.sum_toArray] using List.le_sum_div_length_of_min?_eq_some_int h
...
theorem sum_le_max_mul_length_of_max?_eq_some_int {xs : Array Int} (h : xs.max? = some x) :
    xs.sum ≤ x * xs.size := by
  rcases xs with ⟨l⟩
  simpa [List.max?_toArray, List.sum_toArray] using List.sum_le_max_mul_length_of_max?_eq_some_int h
...
theorem sum_div_length_le_max_of_max?_eq_some_int {xs : Array Int} (h : xs.max? = some x) :
    xs.sum / xs.size ≤ x := by
  rcases xs with ⟨l⟩
  simpa [List.max?_toArray, List.sum_toArray] using List.sum_div_length_le_max_of_max?_eq_some_int h
